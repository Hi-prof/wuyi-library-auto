...
class SeatActionAuditRepositoryTest {

    @Test
    fun `recordFailure persists latest seat action diagnosis`() {
        val repository = SeatActionAuditRepository(FakeSharedPreferences())
        ...
        assertThat(actual.actionLabel).isEqualTo("取消预约")
        assertThat(actual.outcomeLabel).isEqualTo("执行失败")
        assertThat(actual.requestUrl).contains("cancelBook")
        assertThat(actual.message).contains("请重新登录")
    }
    ...
    fun `recordSuccess replaces previous failure`() {
        ...
        repository.recordFailure(
            studentId = "20231121130",
            actionLabel = "取消预约",
            requestUrl = "https://example.com/cancel",
            message = "旧错误",
        ...
        val actual = repository.loadLatest()
        checkNotNull(actual)
        assertThat(actual.outcomeLabel).isEqualTo("执行成功")
        assertThat(actual.message).isEqualTo("取消预约成功")
    }
